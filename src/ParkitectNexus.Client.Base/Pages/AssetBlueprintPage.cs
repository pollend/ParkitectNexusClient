...
{
    public class AssetBlueprintPage : HBox, IPresenter, IPageView
    {
        ...
        public AssetBlueprintPage(ILogger logger, IParkitect parkitect,IWebsite website)
        {
            _website = website;
            ...
            _tileView = new TileView (logger,  getBlueprintTiles);// token => getBlueprintTiles(token));
        ...
        {
            if (e.Asset.Type == AssetType.Blueprint)
        ...
        private IEnumerable<Tile> getBlueprintTiles(CancellationToken token)
        {
            ...
            try
            {
                foreach (var asset in _parkitect.Assets[AssetType.Blueprint])
                {
                    ...
                    {
                        _logger.WriteLine($"Failed to load asset tile for {AssetType.Blueprint} {asset.Name}.");
            ...
            {
                _logger.WriteLine($"Failed to load asset tiles for {AssetType.Blueprint}.");
                _logger.WriteException(e);
            }
            return tiles;
        }

        public string DisplayName => "Blueprints";
