...
{
    public class AssetSavegamePage : HBox, IPresenter, IPageView
    {
        ...
        public AssetSavegamePage(ILogger logger, IParkitect parkitect,IWebsite website)
        {
            _website = website;
        ...
        {
            if (e.Asset.Type == AssetType.Savegame)
        ...
        private IEnumerable<Tile> getBlueprintTiles(CancellationToken token)
        {
            ...
            try
            {
                foreach (var asset in _parkitect.Assets[AssetType.Savegame])
                {
                    ...
                    {
                        _logger.WriteLine($"Failed to load asset tile for {AssetType.Savegame} {asset.Name}.");
            ...
            {
                _logger.WriteLine($"Failed to load asset tiles for {AssetType.Savegame}.");
                _logger.WriteException(e);
            }
            return tiles;
        }

        public string DisplayName => "Savedgames";
